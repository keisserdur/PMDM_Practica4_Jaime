 ...
 */
public class ProviderOutgoing extends ContentProvider {
    ...
    static{
        ...
        convierteUri2Int.addURI(Contract.TableOutgoing.AUTHORITY, Contract.TableOutgoing.TABLEOUTGOING, CANCION);//Le damos la instrucción de qué hacer a la URI
        convierteUri2Int.addURI(Contract.TableOutgoing.AUTHORITY, Contract.TableOutgoing.TABLEOUTGOING + "/#", CANCION_ID);
    }
    ...
    public String getType(Uri uri) {//Devuelve el tipo mime que corresponde a la uri con que se ha llamado
        switch (convierteUri2Int.match(uri)){
            case CANCION:
                return Contract.TableOutgoing.MJLTIPLE_MIME;
            case CANCION_ID:
                return Contract.TableOutgoing.SINGLE_MIME;
            default:
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        long rowId = db.insert(Contract.TableOutgoing.TABLEOUTGOING, null, values);
        if (rowId > 0) {
            //Si se ha insertado el elemento correctamente, entonces devolvemos la uri del elemento que se acaba de insertar
            Uri uri_actividad = ContentUris.withAppendedId(Contract.TableOutgoing.CONTENT_URI, rowId);
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case CANCION: //Muchos clientes
                affected = db.delete(Contract.TableOutgoing.TABLEOUTGOING,selection,selectionArgs);
                break;
            case CANCION_ID: //Un sólo cliente
                long idActividad = ContentUris.parseId(uri);
                affected = db.delete(Contract.TableOutgoing.TABLEOUTGOING, Contract.TableOutgoing._ID + "= ?" , new String [] {idActividad + ""});
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        switch (convierteUri2Int.match(uri)) {
            case CANCION:
                affected = db.update(Contract.TableOutgoing.TABLEOUTGOING, values, selection, selectionArgs);
                break;
            case CANCION_ID:
                ...
                affected = db.update(Contract.TableOutgoing.TABLEOUTGOING, values,
                        Contract.TableOutgoing._ID + "= ?" , new String [] {idActividad});
                break;
            default:
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        ...
        switch (match) {
            case CANCION:
                ...
                c = db.query(Contract.TableOutgoing.TABLEOUTGOING, projection, selection, selectionArgs, null, null, sortOrder);
                break;
            case CANCION_ID:
                // Consultando un solo registro basado en el Id del Uri
                long idActividad = ContentUris.parseId(uri);
                c = db.query(Contract.TableOutgoing.TABLEOUTGOING, projection, Contract.TableOutgoing._ID + "= ? " , new String [] {idActividad + ""},
                        null, null, sortOrder);
                break;
            default:
                throw new IllegalArgumentException("URI no soportada: " + uri);
        }
        c.setNotificationUri(getContext().getContentResolver(), Contract.TableOutgoing.CONTENT_URI);
        return c;
    }
